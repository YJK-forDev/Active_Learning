...
class ScannetDataset():
  #npoints 원래 값은 8192개
    ...
    def generate_chunks_for_query(self):
      """
      ...
      for scene_id in tqdm(self.scene_list):
          ...
          # query scene 용용 (5퍼센트만 초기 추출해서 학습함)
          """
          chunk = scene
          choices = np.random.choice(chunk.shape[0],self.npoints, replace=False)
          
          

          # 아래가 train scene 사용했을때~!
      ...
      print("*******")
      for i in self.chunk_data.values():
        print(len(i))
      print("done!\n")

    def generate_chunks_for_validation_all(self):
      """
          note: must be called before training
      """
      np.random.seed(1024)
      ...
      for scene_id in tqdm(self.scene_list):
          ...
          """
          chunk = scene
          choices = np.random.choice(chunk.shape[0],chunk.shape[0], replace=False)
    ...
    def generate_chunks_initial_train_data(self):
        """
            note: must be called before training
        """
        np.random.seed(1024)
        ...
        for scene_id in tqdm(self.scene_list):
            ...
            # query scene 용용 (5퍼센트만만 초기 추출해서 학습함)
            chunk = scene
            print("총 개수는 ~~~~~~~~~~~ ", int(chunk.shape[0]*0.05))
            choices = np.random.choice(chunk.shape[0], int(chunk.shape[0]*0.15), replace=True)
            print("총 개수는 ~~~~~~~~~~~ ", int(chunk.shape[0]*0.05))
...
class ScannetDataset_update_New_TrainData():
    ...
    def __init__(self, phase, train_scene_list, query_scene_list, new_labeled_data, num_classes=21, npoints=8192, is_weighting=True, use_multiview=False, use_color=False, use_normal=False):
        self.phase = phase
        assert phase in ["train", "val", "test", "query"]
    ...
    def __getitem__(self, index):
        ...
        # 아래래 부분분 수정해야야 함함 (유저에게게 받은은 인풋풋 + 초기기 training 데이터터 모두두 쓸쓸 수수 있도록)
        scene_data = self.chunk_data[scene_id]
    ...
    def generate_chunks(self):
        ...
        for scene_id in scene_list_wQuery:
            ...
            # 이 때 user에게게 받은은 input을을 포함해서서 초기기 training에에 썼던던 데이터를를 함께께 포함하도록록 수정해야야 함함 (랜덤하게게 8192개개 뽑는게 아니라)
            choices = np.random.choice(chunk.shape[0], self.npoints, replace=True)
            chunk = chunk[choices]
            self.chunk_data[scene_id] = chunk
