...
def get_scene_list(path, random_num=0, train=False):
    ...
    if (train==True):
        ...
        # 아래의 query_scene_list는 사용하지 않음
        #query_scene_list = [random.choice(list(set(scene_list) - set(train_scene_list)))] # 나머지 scene에서 하나만 랜덤하게 추출함, 하지만 여기서는 사용하지 않음!
        query_scene_list = []
...
def train(args): # 초기 학습 + active learning 수행함
    ...
    if args.debug:
        ...
        train_scene_list, query_scene_list = get_scene_list(CONF.SCANNETV2_TRAIN, 2, True) # train.txt에 저장되어 있는 scene들을 모두 가져옴, 2는 숫자 뭐 들어가도 상관없게 코딩해놓음
