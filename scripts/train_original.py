...
def get_dataloader(args, scene_list, phase):
    if args.use_wholescene:
        dataset = ScannetDatasetWholeScene(phase, scene_list, is_weighting=not args.no_weighting, use_color=args.use_color, use_normal=args.use_normal, use_multiview=args.use_multiview)
...
def get_scene_list(path, random_num=0, train=False):
    scene_list = []
    with open(path) as f:
        for scene_id in f.readlines():
            scene_list.append(scene_id.strip())
    if (train==True):
        train_scene_list = random.sample(scene_list,random_num) # 랜덤하게 random_num개의 scene을 추출함
        query_scene_list = random.choice(list(set(scene_list) - set(train_scene_list))) # 나머지 scene에서 하나만 랜덤하게 추출함
...
def train(args):
    ...
    if args.debug:
        ...
        train_scene_list, query_scene_list = get_scene_list(CONF.SCANNETV2_TRAIN, 2, True) # train.txt에 저장되어 있는 scene들 중 랜덤하게 2개를 추출함
        # 쿼리의 대상이 될 랜덤한 1개의 scene을 추출함 (train_scene_list에는 없는 것)
